...
export async function getDevices() {
  ...
  const lastReadings = await Promise.all(
    rows.map(async (row) => await getLastDeviceReading(row.id))
  );

  let readingsDatesMap = lastReadings.reduce((acc, reading) => {
    acc[reading.data?.device_id] = reading.created_at;
    return acc;
  }, {});

  return rows.map((row) => {
    const lastReading = readingsDatesMap[row.id];
    return {
      id: row.id,
      board: row.board,
      name: row.name,
      fields: row.fields,
      last_seen_at: lastReading,
    };
  });
}
...
export async function getDeviceConfig(deviceId) {
  ...
  const lastReading = await getLastDeviceReading(row.id);
  return {
    ...row,
...
export async function getDataSummaryForDevice(deviceId) {
  const { rows } = await pool.query(
    `SELECT id, created_at, date, fields, ai_summary, timeframe
     FROM sensor_summary
     WHERE device_id = $1
     ORDER BY date DESC`,
    [String(deviceId)]
